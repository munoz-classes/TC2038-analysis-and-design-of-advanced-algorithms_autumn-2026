...
*   **Meta:** Maximizar el beneficio. Necesitamos una heurística que nos diga "cuánto es lo máximo que podríamos ganar" si seguimos por un camino.
...
# Diagrama de Flujo: Inicialización y Control

```elixir
Para visualizar el flujo general del algoritmo sin entrar en código:
1. [Inicio] $\rightarrow$ Ordenar objetos por beneficio/peso.
2. [Inicializar] $\rightarrow$ max_profit = 0. Crear Nodo Raíz (vacío). Insertar Raíz en la Cola activa.
3. [Condición de Paro] $\rightarrow$ ¿La Cola está vacía?SÍ: 
   * Ir a [Fin] (Retornar max_profit).
   * NO: Extraer Nodo U de la Cola.
4. [Verificación de Nivel] $\rightarrow$ ¿El Nodo U es una hoja (nivel = n-1)?
   * SÍ: Descartar y volver al paso 3.
   * NO: Pasar a la fase de [Ramificación] para el siguiente objeto.
```

---

# Diagrama de Flujo: Ramificación y Poda

Continuación del flujo desde el nodo U hacia el objeto $i$:

```elixir

1. [Hijo Izquierdo: INCLUIR objeto $i$]
   * Crear Nodo V. Sumar peso y beneficio.
   * ¿V.peso $\leq W$? (Factibilidad)
      * SÍ: ¿V.beneficio > max_profit? $\rightarrow$ Actualizar max_profit.
      * Calcular Cota(V). ¿Cota(V) > max_profit?
        * SÍ: Encolar V. (Prometedor).
        * NO: PODAR.
2. [Hijo Derecho: EXCLUIR objeto $i$]
   * Crear Nodo W. Mantener peso y beneficio iguales a U.
   * Calcular Cota(W). ¿Cota(W) > max_profit?
     * SÍ: Encolar W. (Aún promete).
     * NO: PODAR.
3.  Volver al paso 3 del flujo principal.
...
**Problema:** Un mensajero tiene una mochila con una capacidad máxima $W = 15$ kg. Debe decidir qué paquetes llevar para maximizar el valor de la entrega. 

Los paquetes disponibles son:
* **P1:** Peso = 4 kg, Valor = 10
* **P2:** Peso = 2 kg, Valor = 4
* **P3:** Peso = 9 kg, Valor = 27
* **P4:** Peso = 6 kg, Valor = 15
* **P5:** Peso = 5 kg, Valor = 14

**Instrucción:** Dibuja a mano el árbol de espacio de estados aplicando Ramificación y Poda (estrategia FIFO / Búsqueda en Anchura).

---

# Ejercicio 1: Tips de Solución

Antes de empezar a dibujar el árbol a ciegas, sigue estos pasos:

1. **Calcula el Ratio:** Añade una columna a tu tabla y calcula $v_i / w_i$ para cada paquete.
2. **¡Ordena primero!** Si no ordenas los paquetes de mayor a menor ratio antes de empezar, tu cota fraccional será débil y terminarás dibujando casi todo el árbol (fuerza bruta disfrazada).
3. **Traza inicial:** Calcula la cota del nodo raíz. Ese número te dirá el máximo teórico absoluto del problema.
4. **Cuidado con las fracciones:** Recuerda que la fracción solo se aplica al *último* objeto que rompe la capacidad de la mochila al calcular la cota.

---

# Ejercicios Prácticos: Tarea 2
...
# Lectura Previa Obligatoria (Tarea 3)
